...
const FoodPartnerRegister = () => {
  ...
  const handleSubmit = (e) => { 
    ...
    axios.post(`${import.meta.env.VITE_API}/api/auth/food/register`, {
      name:businessName,
      contactName,
      phone,
      email,
    ...
    <div className="auth-page-wrapper">
      <div className="auth-card" role="region" aria-labelledby="partner-register-title">
        ...
        <nav className="auth-alt-action" style={{marginTop: '-4px'}}>
          <strong style={{fontWeight:600}}>Switch:</strong> <Link to="/user/register">User</Link> • <Link to="/food/register">Food partner</Link>
        </nav>
        ...
        <div className="auth-alt-action">
          Already a partner? <Link to="/food/login">Sign in</Link>
        </div>
      </div>
    </div>
  );
